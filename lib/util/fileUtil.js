...
var writeFileSync = exports.writeFileSync = function(p, text, charset, needBak){
    if(fu.exist(p)) {
        if(needBak) {
            fu.rename(p, p + '.tmp');
        }
    ...
    //write file
    fs.writeFileSync(p, text, charset == 'gbk' ? '': charset);

    if(needBak && fu.exist(p + '.tmp')) {
        if(fu.exist(p + '.bak')) {
            fu.delete(p + '.bak');
        }
        fu.rename(p + '.tmp', p + '.bak');
    }
};