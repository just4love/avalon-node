...
module.exports = {
    ...
    getConfig: function(root, cb){
        ...
        async.waterfall([
            ...
            function(webroot, callback) {
                //处理webroot不正确的情况，比如b2b的目录结构
                if(_.isEmpty(app['subModule']) && fs.existsSync(path.join(root, 'deploy'))) {
                    //b2b目录处理
                    app['webRoot'] = path.join(root, 'deploy');
                    finder.findSubModuleAnother(app['webRoot'], function(err, result) {
                        app['subModule'] = result;
                        callback(err, app['webRoot']);
                    });
                } else {
