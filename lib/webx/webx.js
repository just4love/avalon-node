var fs = require('fs'),
    ...
    finderRouter = require('../finder/finderRouter'),
...
var getTemplatesInFileSync = function(p, baseModule, config){
    ...
    function getTemplates(p, innerModule){
        var t = p.split('/'),
            type = t.shift(),
            pathPrefix = type == 'vmcommon' ? config[type]: config['subModule'][innerModule][type];

        var vmPath = path.join(pathPrefix, t.join(path.sep));
        var c = fileUtil.getFileContentSync(vmPath, config['encoding'] || 'gbk', null);

        if(baseModule === innerModule) {
            if(!_.isNull(c)) {
                content[p] = c;
            } else {
                content[p] = null;
            ...
            //module不同的时候，key要加上module
            if(!_.isNull(c)) {
                content[innerModule + ':' + p] = c;
            } else {
                content[innerModule + ':' + p] = null;
            }
        }
...
var getTemplatesJSONSync = function(p, baseModule, config){
    ...
    function getTemplatesTree(p, innerModule, tree){
        ...
        if(depCache[p]) {
            ...
            var vmPath = path.join(pathPrefix, t.join(path.sep));
            var text = fileUtil.getFileContentSync(vmPath, config['encoding'] || 'gbk');
        ...
        _.each(includes, function(include){
            if(include.indexOf(':') != -1) {
                var includeSplits = include.indexOf(':');
                getTemplatesTree(includeSplits[1], includeSplits[0], tree[p]);
            } else {
                getTemplatesTree(include, innerModule, tree[p]);
            }
        });
    }
...
module.exports = {
    ...
    getConfig: function(root, type, cb){
        ...
        finderRouter.route(app, type, function(err, appCfg){
