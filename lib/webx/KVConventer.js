    ...
    if(s) {
        ...
        vars && vars.forEach(function(v){
            if(s.indexOf(v + '(') == -1) {
                ...
                if(!/.*(Module|Util|velocityCount|Server|tool|Tool|screen_placeholder|Control|rundata|csrfToken).*/.test(objString.split('.')[0])) {
...
var mergeVars = function(vars, setVars, forVars, templateName, result, controlParams){
    //解析变量，放到对象中
    _.each(vars.reverse(), function(v){
        var vs = v.split('.');
        setValue(result, vs);

        if((_.isString(forVars[vs[0]]) && _.isEmpty(forVars[vs[0]]))
            || (_.isString(setVars[vs[0]]) && _.isEmpty(setVars[vs[0]]))
            || (_.isString(controlParams[vs[0]]) && _.isEmpty(controlParams[vs[0]]))) {
            //在这里的都是常量，可以去掉
            delete result[vs[0]];
        }
        ...
        if(controlParams[vs[0]]) {
            moveValue(result, vs[0], controlParams[vs[0]].split('.'));
        }
    });
...
//搜集control中的参数
var getControlVars = function(s, result){
    if(s) {
        //先匹配出control那整句话，再从里面取数据
        var cs = s.match(/setTemplate\(.*\)/g);
        cs && cs.forEach(function(v){
            // => ['("vmcommon/s/3.1/css.vm")', '("cssExtra", "p/c2c/header-top-min.css,tbsp/tbsp.css")', '("cxx", $abc)']
            var vars = v.match(/\(.*?\)/g);
            var depKey = vars.shift().match(/[^"()]+.vm/);
            result[depKey] = result[depKey] || {};

            _.each(vars, function(vs){
                //e.g. cssExtra,p/c2c/header-top-min.css,tbsp/tbsp.css
                var t = vs.replace(/["'\(\)\s]/g, '').split(',');
                if(t.length) {
                    if(t[1].indexOf('$') != -1) {
                        result[depKey][t[0]] = getVars(t[1])[0];
                    } else {
                        result[depKey][t[0]] = '';
                    }
                }
